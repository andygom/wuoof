...
class Booking {
	...
	public function book_new_service($client_id, $dog_sitter_id, $pet_id, $service_id, $appointment_date, $num_events, $paid_amount, $status_payment){
		global $extras_class;
		global $services_class;
		
		$response_array = $extras_class->response("false","e1","Hubo un problema interno (500).", "both");

		$client_id = mysqli_real_escape_string($this->conn,$client_id);

		$query = "SELECT * FROM bookings WHERE client_id = '$client_id' AND status !='finished'";
		$result = mysqli_query($this->conn, $query);

		if(mysqli_num_rows($result)){
			$response_array = $extras_class->response("false","e1"," Id regis Tienes una reservacion en proceso.", "both");
		} else {
			...
			$dog_sitter_id = mysqli_real_escape_string($this->conn,$dog_sitter_id);
			...
			$num_events = mysqli_real_escape_string($this->conn,$num_events);
			$paid_amount = mysqli_real_escape_string($this->conn,$paid_amount);
			$status_payment = mysqli_real_escape_string($this->conn,$status_payment);
			$date = date('m/d/Y h:i:s a', time());

			$sql = "INSERT INTO bookings (booking_id,client_id,dog_sitter_id,pet_id,service_id,appointment_date,	num_events,paid_amount,status_payment,status, creation_date) VALUES ('$booking_id','$client_id','$dog_sitter_id','$pet_id','$service_id','$appointment_date','$num_events','$paid_amount','$status_payment','pending','$date')";

			if (mysqli_query($this->conn,$sql)) {					
				
				$response_array = $extras_class->response("true","sc1","Registro. Se envió la solicitud a los paseadores.", "console");
	...
	public function confirm_booking_request($booking_id){
		global $extras_class;
		global $services_class;
	...
	public function update_booking_status($booking_id, $status, $status_payment){
		...
		if ($extras_class->record_exists('bookings','booking_id',$booking_id)) {
			...
			$sql = "UPDATE bookings SET status = '$status', status_payment = '$status_payment' WHERE booking_id = '$booking_id';";
			if (mysqli_query($this->conn,$sql)) {
				$response_array = $extras_class->response("true","s1","Se actualizó el estatus exitósamente.", "both");
			} else {
				$response_array = $extras_class->response("false","ec1","Hubo un problema para actualizar el estatus de la reservación.", "both");
	...
	public function get_booking_data($booking_id){
		...
		$query = "SELECT bookings.booking_id, bookings.client_id, bookings.dog_sitter_id, bookings.status, users.name, users.img_url, pets.pet_name, pets.pet_img_url FROM bookings INNER JOIN users ON bookings.client_id=users.user_id INNER JOIN pets ON bookings.client_id = pets.user_id WHERE booking_id = '$booking_id'";
		...
		if (mysqli_num_rows($result)) {
		  while($row = mysqli_fetch_assoc($result)) {
			...
			$data_list_array[] = array("booking_id" => $row['booking_id'],"client_id" => $row['client_id'], "dog_sitter_id" => $row['dog_sitter_id'],"status" => $row['status'],"name_user" => $row['name'],"img_url" => $row['img_url'],"pet_name" => $row['pet_name'],"img_url_pet" => $row['pet_img_url']);
		  }

		  //$sql = "SELECT bookings.booking_id, bookings.client_id, bookings.dog_sitter_id, bookings.status, users.name, users.img_url, pets.pet_name, pets.pet_img_url FROM bookings INNER JOIN users ON bookings.client_id=users.user_id INNER JOIN pets ON bookings.client_id = pets.user_id WHERE booking_id = '$booking_id'";

		  $id_user = $data_list_array[0]["dog_sitter_id"];

		  $sql = "SELECT users.name, users.img_url,user_services.price, services_list.name_service FROM users INNER JOIN user_services ON users.user_id=user_services.user_id INNER JOIN services_list ON user_services.service_id=services_list.service_id WHERE users.user_id = '$id_user' ";

			$result2 = mysqli_query($this->conn, $sql);

			while($row = mysqli_fetch_assoc($result2)) {
			  
			$data_list_array2[] = array("name_paseador" => $row['name'],"img_url" => $row['img_url'], "price" => $row['price'],"name_service" => $row['name_service']);
		  	}

		  	//$resultado= array(array_merge($data_list_array,$data_list_array2));

		  	$resultado = array_merge($data_list_array, $data_list_array2);

		  $response_array = $extras_class->response("true","sc1","La información de los servicios se obtuvo con éxito.", "console");
		  //$response_array[] = array('data_booking' => $data_list_array);
		  $response_array[] = array('booking_data' => $data_list_array2);
		} else {
